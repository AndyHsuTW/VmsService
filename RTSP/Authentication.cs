...
namespace Rtsp
{
    ...
    public class Authentication
    {
        ...
        public static Authentication GetAuthenticationInfo(Rtsp.Messages.RtspMessage received_message)
        {
            ...
            if (authorization.StartsWith("Basic ", StringComparison.OrdinalIgnoreCase))
            {
                ...
                authInfo = new Authentication(decoded_username, decoded_password, null, Type.Basic);
            }
            else
            {
                _logger.Warn($"Authentication type not supported:{authorization}");
            }
            // Check Username, URI, Nonce and the MD5 hashed Response
            //else if (authorization.StartsWith("Digest ",StringComparison.OrdinalIgnoreCase))
            //{
